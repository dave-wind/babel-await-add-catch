  ...
  const visitor = {
    AwaitExpression(path) {
      ...
      if (this.opts && !isObject(this.opts)) {
        return console.error('[babel-await-add-catch]: options need to be an object.');
      }

      const isTryExpression = path.findParent(p => p.isTryStatement()); // 当父路径已经存在
      ...
      const thisEnvFn = path.findParent(p => p.node.async && (p.isFunctionDeclaration() || p.isArrowFunctionExpression() || p.isFunctionExpression())); // 获取 声明 await 函数的 变量

      const declarIdPath = path.getSibling('id');
      const variableDeclaration = declarIdPath && declarIdPath.node ? declarIdPath.node.name : ''; // 当申明过变量 且 作用域内 不存在该变量
      ...
      if (variableDeclaration && !thisEnvFn.scope.hasBinding('$_' + variableDeclaration)) {
        ...
        for (let i in thisEnvFn.scope.bindings) {
          let item = path.scope.bindings[i]; // 匹配作用域变量 进行替换
